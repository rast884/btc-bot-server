...
async function placeBet(rid) {
  ...
  if (sig.skip) {
    ...
    state.history.unshift({
      ...
      endPrice: null, window: fmtWindow(rid), result: 'skip',
      pnl: 0, balanceAfter: state.wallet.balance,
...
function listenForCommands() {
  ...
  STATE_REF.child('strategy').on('value', (snap) => {
    if (state && snap.val()) { state.strategy = snap.val(); console.log('[STRAT] Strategy:', state.strategy); }
  });

  STATE_REF.child('skipMode').on('value', (snap) => {
    if (state && snap.val() !== null) { state.skipMode = snap.val(); console.log('[SKIP]', state.skipMode ? 'Skip ON' : 'Skip OFF'); }
  });
