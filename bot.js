...
let _deltaEntry = { elapsed:0, priceDelta:0, score:0 };
...
async function fetchOrderFlow() {
  ...
  try {
    ...
    let signal='neu', score=0;
    if      (imbalance> 0.20){signal='bull';score= 3.5;}
    else if (imbalance> 0.10){signal='bull';score= 2.0;}
    else if (imbalance<-0.20){signal='bear';score=-3.5;}
    else if (imbalance<-0.10){signal='bear';score=-2.0;}
    _orderFlow={imbalance,signal,score,fetched:Date.now()};
  } catch(e){}
}
...
function calcDeltaEntry(buf, startPrice, roundRemainMs) {
  const elapsed=5*60*1000-roundRemainMs;
  if(!buf||buf.length<5||!startPrice) return { score:0, elapsed:Math.floor(elapsed/1000) };
  const curPrice=buf[buf.length-1];
  const priceDelta=((curPrice-startPrice)/startPrice*100);
  const absDelta=Math.abs(priceDelta);
  let score=0;
  if(elapsed>=60000&&elapsed<=240000){
...
function aiSignal(klines) {
  ...
  // 9. Delta Entry
  const rem2 = state.lastRoundId ? (state.lastRoundId+5*60*1000-Date.now()) : ROUND_MS;
...
async function resolveBet() {
  ...
  _roundStartPrice = 0; // сбрасываем для следующего раунда
  if (!wsConnected) await fetchPriceFallback();
